...
static int init(struct ao *ao, char *params)
{
    ...
    ca_msg(MSGT_AO, MSGL_V, "init([%dHz][%dch][%s][%d])\n",
        ao->samplerate, ao->channels, af_fmt2str_short(ao->format), 0);
    ...
    inDesc.mChannelsPerFrame = ao->channels;
    ...
    ao->channels   = inDesc.mChannelsPerFrame;
...
static int OpenSPDIF(struct ao *ao)
{
    ...
    ao->channels = p->stream_format.mChannelsPerFrame;
