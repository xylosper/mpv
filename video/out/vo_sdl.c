...
const struct formatmap_entry formats[] = {
    ...
    {SDL_PIXELFORMAT_RGBX8888, IMGFMT_RGBA, -1}, // has no alpha -> bad for OSD
    {SDL_PIXELFORMAT_BGRX8888, IMGFMT_BGRA, -1}, // has no alpha -> bad for OSD
...
#else
    {SDL_PIXELFORMAT_RGBX8888, IMGFMT_ABGR, -1}, // has no alpha -> bad for OSD
    {SDL_PIXELFORMAT_BGRX8888, IMGFMT_ARGB, -1}, // has no alpha -> bad for OSD
...
static void generate_osd_part(struct vo *vo, struct sub_bitmaps *imgs)
{
    ...
    if (imgs->bitmap_id != sfc->bitmap_id || !sfc->tex) {
        ...
        if (r < 0) {
            mp_msg(MSGT_VO, MSGL_ERR, "[sdl] OSD bitmaps do not fit on "
            ...
            sfc->tex = NULL;
            mp_msg(MSGT_VO, MSGL_V, "[sdl] Allocating a %dx%d surface for "
